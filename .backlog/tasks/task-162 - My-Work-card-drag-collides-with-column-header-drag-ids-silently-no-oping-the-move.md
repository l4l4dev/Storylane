...
- [ ] #2 Today -> Todo (and every other cross-column card drag) succeeds via a real multi-step pointer drag, verified in the browser
- [ ] #3 Column-header dragging (TASK-148) is unaffected: still only responds to its own grip handle
- [ ] #4 Existing my-work-sections.test.tsx drag tests pass; a regression test covers a card drag resolving away from a column id
<!-- AC:END -->
