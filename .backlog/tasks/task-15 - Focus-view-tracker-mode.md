...
<!-- SECTION:NOTES:BEGIN -->
Spec gap resolved with Mika before implementing: Focus view column list (spec/screens.md) never mentioned where rejected-state stories render. Decided: In progress (same reasoning as Kanban views Rejected column - still needs the Restart transition). Documented in spec/screens.md.
