...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 Migration(s) implement my_work_columns + the my_work_story_state reshape exactly as doc-15 specifies (composite FK, column-list SET NULL, today_position check, data conversion)
- [x] #2 project_my_work_mapping dropped in a NEW forward-only migration; Settings 'My Work sync' section, my-work-mapping-broken-banner, and all mapping reads/writes removed from the web app
- [x] #3 'Doing' free column seeded for existing users and created at signup (handle_new_user full replacement)
- [x] #4 database.types.ts regenerated; my-work-data-model integration test rewritten for the new shapes
- [x] #5 rls-security-reviewer pass on the migrations
