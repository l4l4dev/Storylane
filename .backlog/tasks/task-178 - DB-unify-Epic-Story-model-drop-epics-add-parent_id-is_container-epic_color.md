...
Slice 3 done + committed (a1e3dc2): full Web compile sweep. Removed epics-table/epic_id across board (badge, filter, draft-card field + prop chain), story detail (epic field -> parent_id preserved across autosave, no editor until TASK-184), /epics (placeholder; epics CRUD UI + actions deleted with the table), board/iterations/my-work/story pages, and all affected tests. update_story/create callers now send p_parent_id. VERIFIED: tsc 0 errors, eslint clean, pnpm test 703 passed / 211 integration skipped / 0 failed; both migrations apply clean via supabase db reset. DEFERRED (flagged): iOS repository/model layer still carries epic (AC#4 mentions iOS) - untouched per the project's Web-first rule; suggest moving that clause to the iOS port task. promote_story_to_epic, its UI, promote.integration.test and the personal-project-seal-seams promote block are intentionally left for TASK-181.
<!-- SECTION:NOTES:END -->
