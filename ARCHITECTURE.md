...
The trigger watches `state_id`, `iteration_id`, `points` and `assignee_id` independently on the same `UPDATE` (a single write can change several — e.g. scheduling a story into an iteration's category for the first time — and each gets its own row), so `finalize_iteration`'s rollover reparent and an ordinary Backlog↔Current reschedule (`move_story_board`) both record a `story.iteration_changed` row automatically, with no per-caller logging of their own. The same independence covers writes no caller performs: the composite assignee FK's `ON DELETE SET NULL` unassigns a removed member's stories, and that cascade is logged like any other update. The watched list is duplicated in `spec/screens.md` "Conflict & failure rules" — adding a column means changing both.
