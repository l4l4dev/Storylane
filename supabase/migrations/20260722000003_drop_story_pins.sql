-- ============================================================
-- TASK-131 (doc-14): drop story_pins — its role (the personal "Today" marker)
-- was folded into my_work_story_state.is_today by TASK-130's migration. This
-- was split out of that migration so main kept compiling/passing between the
-- two tasks (the TS that reads story_pins is removed in this same task).
--
-- Recreate the two SECURITY DEFINER RPCs that referenced story_pins BEFORE
-- dropping it: move_story_to_project loses its pin carry-over entirely (a
-- moved story is recreated under a new id, and per-user marks don't follow —
-- doc-14 keeps My Work marks personal to the original story), and
-- remove_member swaps its pin purge for a my_work_story_state purge (same
-- reasoning: a member leaving shouldn't keep, or a re-invite revive, their My
-- Work marks on that project's stories).
-- ============================================================

-- move_story_to_project: verbatim from 20260720000005 minus the story_pins
-- carry-over block.
...
-- remove_member: verbatim from 20260720000005 with the story_pins purge
-- replaced by a my_work_story_state purge.
...
-- recreate story_pins (+ RLS/grants) from 20260720000004, then restore
-- move_story_to_project and remove_member from 20260720000005 verbatim.
