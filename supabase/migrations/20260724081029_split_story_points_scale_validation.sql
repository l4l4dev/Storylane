-- ============================================================
-- TASK-181 follow-up (/code-review, TASK-182 pass): validate split_story's
-- child points against the project's point scale.
--
-- split_story stored `(child->>'points')::int` verbatim, unlike update_story /
-- move_story_to_project / copy_story_to_project which all clamp an off-scale
-- value to NULL. The RPC is a trust boundary — a member calling it directly (or
-- a client bug) could store an off-scale value (e.g. 999 on a fibonacci
-- project), corrupting velocity and the container roll-up. Mirror move/copy's
-- clamp-to-NULL for each child.
--
-- create-or-replace of 20260724071826's split_story — verbatim except the
-- point-scale read + per-child clamp. Grants preserved across CREATE OR REPLACE.
-- ============================================================
...
-- (restore split_story from 20260724071826_epic_story_unification_split_story.sql)
