...
- [TASK-172 full-page width gap](project-task-172-story-full-page-width.md) — max-w-4xl single-column bump likely doesn't satisfy AC#4's "redesigned/full width" wording; two-column split direction recorded, awaiting owner call
...
- [TASK-186 icebox accordion drag design](task186-icebox-accordion-drag-design.md) — 2026-07-24 verdict: don't widen ListZoneId, extend containers with epic:<id> keys; AC#2 needs no new RPC (parent_id unchanged per doc-18 §9), AC#3 needs move_story_board extended w/ parent_id delta, AC#1 (container row reorder) split out — different position scope
- [TASK-186 post-implementation review](task186-post-implementation-review.md) — approve-with-fix: cross-zone silent revert is spec-sanctioned, but blanket isAllowedMove rejection also kills same-nest reorder (dead control, no new RPC needed — move_story_board ignores parent_id)
- [TASK-192 set_story_parent + attach UX verdicts](task192-set-story-parent-verdict.md) — RPC approved (RLS premise stale); attach UX ok, but centre-collision on nested rects + child rows inside a drag-handle parent need fixes
- [TASK-188 container reorder verdict](task188-icebox-container-reorder-verdict.md) — 2026-07-25 corrects TASK-186's wrong scope claim: no new RPC/scope/migration, AC#2 needs rewording not loosening, pure client wiring
